public class MadLibs {
    /*
    This java program generates a mad libbed story
    Author: Narmadha
    Date: 20-5-2021
    */
    public static void main(String[] args){
        String name1 = "Monica";
        String adjective1 = "Dizzy";
        String adjective2 = "Long";
        String adjective3 = "Loud";
        String verb1 = "sing";
        String noun1  = "Children";
        String noun2 = "Carrots";
        String noun3 = "Christmas carol";
        String noun4 = "Children";
        String noun5 = "Pool";
        String noun6 = "Ant's'";
        String name2 = "Chandler";
        int number = 17;
        String place1 = "Manhattan";
        //The template for the story
        String story = "This morning "+name1+" woke up feeling "+adjective1+
                        ". 'It is going to be a "+adjective2+" day!' Outside, a bunch of "+noun1+
                        "s were protesting to keep "+noun2+" in stores. " +
                         "They began to "+verb1+" to the rhythm of the "+noun3+
                        ", which made all the "+noun4+"s very "+adjective3+". Concerned, "+name1+" texted "+name2+
                        ", who flew " +name1+" to "+place1+" and dropped "+name1+" in a puddle of frozen "+noun5+
                        ". "+name1+" woke up in the year "+number+", in a world where "+noun6+"s ruled the world.";
        System.out.println(story);
    }
}
